...
1. [`chcon`](https://linux.die.net/man/1/chcon) Helps in changing the SELinux context of a target.  
2. [`audit2allow`](https://linux.die.net/man/1/audit2allow) Generates SELinux policy containing allow & dontaudit rules. Doesn't covers neverallow exceptions. [`audit2allow.perl`](https://github.com/OpenDarwin-CVS/SEDarwin/blob/master/sedarwin7/src/sedarwin/policycoreutils/audit2allow/audit2allow.perl) hosted at [OpenDarwin-CVS/SEDarwin](https://github.com/OpenDarwin-CVS/SEDarwin/) is a script which requires no external dependencies & runs on any platform unlike morden versions.  
3. [`restorecon`](https://linux.die.net/man/8/restorecon) Restores the default SELinux contexts of the given target.  
4. [`sepolicy-inject`](https://github.com/phhusson/sepolicy-inject) Injects allow rules into binary SELinux kernel policies.
