...
The decoder (inside the playback device) is required to support any combination of these parameters, but the encoder (in the smartphone) may only implement them partially.
...
Of said parameters, the bitpool is especially important as it has a direct influence on bitrate: the higher it is, the higher the bitrate, and hence an improved quality. But exact bitpool value corresponds to exact bitrate only within the same set of other codec's parameters.
...
Android patchset has an additional option to increase bitrate for Enhanced Data Rate 2 mbps devices even further. You can bump the bitrate from 452 kbps to 595 kbps, at the cost of reducing the stability of the transmission in case of congested radio conditions.

Just set the persist.bluetooth.sbc_hd_higher_bitrate variable to 1:

`# setprop persist.bluetooth.sbc_hd_higher_bitrate 1`

595 kbps option is currently merged only in LineageOS 15.1, but not in 16.0.
