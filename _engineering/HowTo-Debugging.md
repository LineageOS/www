...
On Qualcomm devices that have CrashDump enabled, you can use Qualcomm's firehose tool to get a memorydump. As the firehose tool is closed source, we recommend using a rewritten open source version of the tool by Bjoern Kerler, which can be found here at [bkerler/edl](https://github.com/bkerler/edl). You can retreive a memory dump using `edl memorydump`.
