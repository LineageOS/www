...
### ramoops-pmsg
ramoops-pmsg user space accessible version of ramoops. To access these logs prior to last reboot from pstore, you can run **:/ $** `adb shell logcat -b all -L`.
...
Although ramoops and pstore are powerful tools, there are some caveats while using them. Since pstore by default writes the data as buffered, and we typically use it only when a system is about to crash, we see a lot of corruption when retrieving a pstore afterwards.

### `decode_stacktrace.sh`
`decode_stacktrace.sh` is a script bundled with every linux kernel source at [`linux/blob/master/scripts/decode_stacktrace.sh`](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/scripts/decode_stacktrace.sh). To use it, you first have to enable `CONFIG_DEBUG_INFO=y` in your kernel config and build the kernel.
...
Here are some custom debugging tools we found over the years that proved helpful for more indepth debugging.

### EDL memorydump (qcom)
On Qualcomm devices that have "crashdump" enabled, you can run `edl memorydump` using Qualcomm's firehose tool.
