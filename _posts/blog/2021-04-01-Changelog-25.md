...
This mean that the same interface of `backup_file` - `restore_file` now supports paths to files under `/vendor`, `/product`, `/system_ext`.
If you need to manipulate a file further, to set permissions or SEContext for example, you can get the real path to it with `get_output_path`.

To enable this functionality, simply declare `# ADDOND_VERSION=3` in your script header.
For a reference adoption, you may refer to this [gist](https://gist.githubusercontent.com/aleasto/9770300b557b719a8d925ac3f3ec646d/raw/7df56287fb39bfb9aee14f102690e443b3bb3a66/addond_example)
...
Crucially, this is fully backwards compatible!
If your script declares `# ADDOND_VERSION=2` you can expect to find the exact same environment as before v3 was introduced.
So if you already had your own logic to mount and access extra partitions, that will continue to work without changes.
