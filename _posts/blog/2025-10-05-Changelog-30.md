...
We've been hard at work rebasing all of our changes and features since Android 16's release in June, and yes you read that right, we're early this year! Android 16's original release mainly contained iterative improvements and some UI/UX refinement, but due to our previous efforts adapting to Google's UI-centric adjustments in Android 12 through 14, we were able to rebase onto Android 16's code-base faster than anticipated.
...
* LineageOS is now nearly `Android.mk` free! Google [announced their move](https://source.android.com/docs/setup/build/make-to-soong) from `make` to `soong` many years ago, pushing developers to migrate from Android.mk to Android.bp, and has started blocking Android.mk in many locations of the source tree.
  ...
  * 0xCAFEBABE also created a build target for converting from [mk to bp](https://review.lineageos.org/c/LineageOS/android_build/+/442552), a WIP feature to assist developers in migrating to soong!
* Both the Charging Control, and Fast Charge Control features have recieved many updates and improvements.
* A new set of ringtones and alarms from Plasm Mobile have been included.
...
With 23.0, developer 0xCAFEBABE has spearheaded an effort to extend compatibility for devices capable of booting the mainline Linux kernel, and we’ve made it easier for maintainers to bring their devices closer to upstream with inheritable common trees. The end result? For now, nothing, but in the future, we will hopefully be able to boot Android on _almost any_ device that is supported by the mainline Linux kernel.
