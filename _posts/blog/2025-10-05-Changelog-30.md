...
We are _way_ ahead of schedule to say the least - and this last year has been a whirlwind to say the
least! But we have remained dedicated to bringing an updated LineageOS based on Android 16 to the
masses!

We've been hard at work rebasing all of our changes and features since Android 16's release in June,
and yes you read that right, we're early this year! Android 16's first release mainly contained
iterative improvements and some UI/UX refinement, but due to our previous efforts adapting to
Google's UI-centric adjustments in Android 12 through 14, we were able to rebase onto Android 16's
code-base faster than anticipated.
...
On top of that, Google’s handling of ASBs has shifted. July was empty for the first time since the
...
security updates are no longer released in full on a monthly basis. Instead, only high and critical
severity CVEs are published in the monthly ASBs, and even then, the underlying patches are often not
made public immediately.

On a quarterly cadence, Google now issues larger security bulletins that include patches for
vulnerabilities originally discovered in prior months. For the latest Android version, these
quarterly bulletins coincide with QPRs (Quarterly Platform Releases), which bundles those security
fixes together with feature updates, but also often are not pushed to AOSP at the time of release.

This is why you've seen the LineageOS 22.2 security patch level remain reflective of August well
into September.
...
Unfortunately, LineageOS 22.2 still supports many devices running `4.4`, `4.9`, and `4.14`. As of
...
* Contributor 0xCAFEBABE has extended support for various VirtIO configurations
  ...
  [the Wiki](https://wiki.lineageos.org/libvirt-qemu). There is even a newer on allowing you to run
...
* LineageOS is now nearly `Android.mk` free! Google
  ...
  * While converting these is a seemingly simple task - in practice it involved countless hours of
    ...
    the writing of this, LineageOS introduces less than 10 Android.mk at a platform level, and many
    of these are in the process of being converted. In short - we're ready for when Google kills
    support for Android.mk.
...
build ocnfigurations, which are similar to the  `emulator` family of targets, but has extra
emulated periphials, so as to act more like a real device! You can view a list of all the differences
...
Additionally, many legacy devices require interpolating libraries that we colloquially refer to as
...
We will allow new LineageOS 21 submissions to be forked to the organization, but we no longer will
...
To upgrade, please follow the upgrade guide for your device by clicking on it [here](https://wiki
lineageos.org/devices/) and then on "Upgrade to a higher version of LineageOS".
...
Though we've had buildable generic targets since 2019, to make LineageOS more accessible to
...
Additionally, an experimental target now exists for QEMU-based virtual machine softwares (libvirt,
UTM, etc) targets. Instructions on building and utilizing these targets can be found on
