...
* Security patches from March 2021 to March 2022 have been merged to LineageOS 15.1 through 19.0.
  * 19.0 builds are currently based on the android-12.0.0_r26 tag, which is the Qualcomm Pixel series tag.
...
The issue lies in the fact that only devices with Linux kernel 4.9 or newer have the needed capabilities to make use of eBPF. Usually, these things can be backported to older kernel versions, but at the moment, even something as close to version 4.9 as 4.4 is proving challenging due to the sheer number of commits and structure changes in BPF's introduction.
...
LineageOS 17.1 builds stopped on January 31st, 2022, to allow space on our infrastructure to begin testing LineageOS 19.0.
