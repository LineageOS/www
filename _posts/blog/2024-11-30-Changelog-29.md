---
layout: post
title: Changelog 29 - Flourishing Fifteen, , Undeniable User-Experience
category: blog
excerpt: Lets have a drink!
author: Nolen Johnson (npjohnson)
---

![hero]({{site.baseurl}}/images/2024-11-30/hero.jpg){: .blog_post_image_full }
...
Maintainers will also be expected to check-in at each QPR to assure that their device still functions will all the changes carried. Don't be disheartened if your device falls out of support during a QPR, it may just take some time to bring it up to par!

Welcome to LineageOS 22.0, based on Android 15 QPR1. Enjoy the new features, and, as always, happy flashing!

### New Features!

* Security patches from March 2024 to November 2024 have been merged to LineageOS 19.1 through 22.0.
* SeedVault and Etar have both been updated to their newest respective upstream version.
* WebView has been updated to Chromium webview_130.0.6723.73.
...
* Contributor demon0000 (Cosmin Tanislav) has reworked our extract utilities, heavily optimizing them. He was able to optimize most devices down from 60 seconds to extract, all the way down to roughly 7 seconds.
  * He also rewrote our extract utilities from scratch in Python, making them far more extensible, and even faster! Maintainers can swap to this implementation at their leisure throughout this cycle.
* Our Setup Wizard has seen adaptation for Android 15, with improved styling, more seamless transitions, and significant amounts of legacy code being stripped out.
* Contributor 0xCAFEBABE also added support for virtIO (VirtualBox/VMWare) targets! Though these aren't supported officially, there is an awesome, comprehensive guide for building and utilizing these targets on [the Wiki](https://wiki.lineageos.org/libvirt-qemu).
...
| Qualcomm SM8450 | [5.10](https://github.com/LineageOS/android_kernel_qcom_sm8450)  | 13   |
...
Overall, we feel that the 22.0 branch has reached feature and stability parity with 21 and is ready for initial release.
...
LineageOS 22.0 will launch building for a decent selection of devices, with additional devices to come as they are marked as both
...
Additionally, an experimental target now exists for virtIO (VirtualBox/VMWare) targets. Instructions on building and utilizing these targets can be found on [the Wiki](https://wiki.lineageos.org/libvirt-qemu).
